...
Original file is located at [REDACTED].
...
print(f"{len(text_pairs)} number of pairs")
...
stirng_chars = strip_chars.replace("]", "")
...
eng_vec= keras.layers.TextVectorization(
    max_tokens=vocab_size, output_mode="int", output_sequence_length=sequence_length
)
spa_vec= keras.layers.TextVectorization(
    max_tokens=vocab_size, output_mode="int", output_sequence_length=sequence_length+1,
    standardize= custom_standardization
)

train_eng_texts= [p[0] for p in train_pairs]
train_spa_texts= [p[1] for p in train_pairs]

eng_vec.adapt(train_eng_texts)
spa_vec.adapt(train_spa_texts)
...
class TransformerEncoder(keras.layers.Layer):
  def __init__(self,embed_dim,dense_dim ,num_heads, **kwargs):
    ...
    self.dense_proj=keras.Sequential(
        [keras.layers.Dense(self.dense_dim, activation='relu'), keras.layers.Dense(embed_dim)]
    )
...
class TransfomrerDecoder(keras.layers.Layer):
  def __init__(self, embed_dim, latent_dim, num_heads, **kwargs):
    super(TransfomrerDecoder, self).__init__(**kwargs)
    ...
    self.dense_proj=keras.Sequential(
        [keras.layers.Dense(self.latent_dim, activation='relu'), keras.layers.Dense(embed_dim)]
    )
    self.layernorm_1= keras.layers.LayerNormalization()
    self.layernorm_2= keras.layers.LayerNormalization()
  ...
  def call(self, inputs, encoder_outputs, mask=None):
    ...
    attention_out_1= self.attention_1(
        query=inputs, value=inputs, key=inputs, attention_mask=causal_mask
    )
    out_1=self.layernorm_1(inputs+attention_out_1)
...
x=TransfomrerDecoder(embed_dim, latent_dim, num_heads)(x,encoded_seq_inputs)
...
transformer.save('/content/drive')